...
class Robot(Parent):
    ...
    # 6_dof constraints the orientation of the contacts,
    # 3_dof only constraint the position
    cType = "_3_DOF"
    offset = [0.0, 0.0, -0.018]  # Contact position in the effector frame

    # Position on the first joint of each limb (in the root frame)
    # in the 'reference' configration,
    # Used by some heuristic when sorting candidates contacts positions
    rLegLimbOffset = [0.1946, 0.0875, 0.0]
    lLegLimbOffset = [0.1946, -0.0875, 0.0]
    rArmLimbOffset = [-0.1946, 0.0875, 0.0]
    lArmLimbOffset = [-0.1946, -0.0875, 0.0]

    normal = [0, 0, 1]  # Contact normal, in the effector frame
    legx = 0.01
    legy = 0.01  # Contact patch size, cannot be 0 even for contact points
